...
export const SidebarChatButton = ({chatItem, active, onClick, onDelete, onEdit}: Props) => {
  ...
  return (
    <div onClick={handleClickButton} className={`flex items-center rounded-md p-3 text-sm cursor-pointer hover:bg-blue-500/10 ${active ? 'bg-blue-500/20' : 'bg-transparent'}`}>
      <div className="mr-3">
      ...
      <div className="flex-1 text-sm overflow-x-hidden">
        ...
        {!editing && 
          <div className="border border-transparent truncate">
            ...
            {deleting && `Excluir "${chatItem.title}`}
          </div>
        }
      </div>
      {active && !deleting && !editing &&
        <div className="flex">
          <div onClick={(e) => { stopPropagation(e); setEditing(true);}} className="cursor-pointer mx-1 opacity-60 hover:opacity-100">
            <IconEdit3 width={16} height={16} />
          </div>
          <div onClick={(e) => { stopPropagation(e); setDeleting(true);}} className="cursor-pointer mx-1 opacity-60 hover:opacity-100">
